  ...
  <header class="sticky bg-yideli-dark top-0 z-50 backdrop-blur-sm transition-all duration-300 shadow-md"
    ...
    <div
      ...
      <div class="flex-shrink-0 me-8">
        <a class="block" href="{{ route('index', ['lang' => $lang]) }}">
          <img class="h-20 w-auto object-contain" src="{{ asset('images/logo_big.jpg') }}" alt="Yideli Logo">
        </a>
      </div>

      <nav class="hidden lg:flex gap-8 text-sm font-medium tracking-wide "
        style="margin-top:32px; color: rgb(239 245 230);">
        <a class="hover:text-gray-300 transition" href="{{ route('index', ['lang' => $lang]) }}">HOME</a>
        <a class="hover:text-gray-300 transition"
          href="{{ route('page.show', ['lang' => $lang, 'slug' => 'about-us']) }}">ABOUT US</a>
        <a class="hover:text-gray-300 transition"
          href="{{ route('page.show', ['lang' => $lang, 'slug' => 'production-process']) }}">PRODUCTION
          PROCESS</a>
        <a class="hover:text-gray-300 transition" href="{{ route('product.index', ['lang' => $lang]) }}">PRODUCT
          DISPLAY</a>
        <a class="hover:text-gray-300 transition" href="{{ route('news.index', ['lang' => $lang]) }}">NEWS</a>
        <a class="hover:text-gray-300 transition" href="{{ route('inquire.form', ['lang' => $lang]) }}">CONTACT
          US</a>
      </nav>
      ...
      <div class="flex items-center gap-6" style="margin-top:32px;color:rgb(239 245 230);">
        <div class="relative" x-data="{
          ...
          <button
            class="flex items-center gap-1 text-sm font-medium hover:text-yideli-base transition focus:outline-none"
            style="color: rgb(239 245 230);" @click="open = !open" @click.outside="open = false"> <span
              class="font-bold" x-text="langs[current]"></span>
          ...
          <div
            class="absolute end-0 mt-2 w-32 bg-white rounded-md shadow-lg border border-yideli-line overflow-hidden z-50"
            ...
            <div class="py-1">
              <template x-for="(label, code) in langs" :key="code">
                <a class="block px-4 py-2 text-sm text-gray-700 hover:bg-yideli-base hover:text-yideli-dark transition-colors"
                  href="javascript:void(0)" @click="switchLanguage(code)"
    ...
    <div class="lg:hidden absolute top-full start-0 w-full bg-yideli-base border-t border-yideli-line shadow-lg"
      x-show="mobileMenu" x-transition x-cloak>
      <div class="flex flex-col p-6 gap-4 text-lg font-serif text-yideli-dark">
        <a class="py-2 border-b border-yideli-line/30" href="{{ route('index', ['lang' => $lang]) }}">HOME</a>
        <a class="py-2 border-b border-yideli-line/30"
          href="{{ route('page.show', ['lang' => $lang, 'slug' => 'about-us']) }}">ABOUT US</a>
        <a class="py-2 border-b border-yideli-line/30" href="{{ route('product.index', ['lang' => $lang]) }}">PRODUCT
          DISPLAY</a>
        <a class="py-2 border-b border-yideli-line/30"
          href="{{ route('page.show', ['lang' => $lang, 'slug' => 'production-process']) }}">PRODUCTION PROCESS</a>
        <a class="py-2 border-b border-yideli-line/30" href="{{ route('news.index', ['lang' => $lang]) }}">NEWS</a>
        <a class="py-2 border-b border-yideli-line/30" href="{{ route('inquire.form', ['lang' => $lang]) }}">CONTACT
          US</a>
      </div>
    </div>
  </header>
  ...
  <footer class="bg-yideli-dark text-white pt-20 pb-10">
    ...
    <div
      ...
      <div class="lg:col-span-2">
        <h3 class="text-2xl font-bold font-serif tracking-wider">
          <img class="h-20 w-auto" src="{{ asset('images/logo_big.jpg') }}" alt="YIDELI">
        </h3>
      ...
      <div>
        ...
        <ul class="space-y-4 text-sm text-white/70">
          <li><a class="hover:text-white transition"
              href="{{ route('page.show', ['lang' => $lang, 'slug' => 'about-us']) }}">About Us</a></li>
          <li><a class="hover:text-white transition"
              href="{{ route('page.show', ['lang' => $lang, 'slug' => 'production-process']) }}">Production
              Process</a></li>
          <li><a class="hover:text-white transition" href="{{ route('product.index', ['lang' => $lang]) }}">Product
              Display</a></li>
          <li><a class="hover:text-white transition" href="{{ route('news.index', ['lang' => $lang]) }}">News</a>
          </li>
          <li><a class="hover:text-white transition" href="{{ route('inquire.form', ['lang' => $lang]) }}">Contact
              Us</a></li>
        </ul>
      </div>
    ...
    <div
      ...
      <div class="flex gap-6 mt-4 md:mt-0">
        ...
        <a class="hover:text-white transition" href="#">Terms of Use</a>
      </div>
    </div>
  </footer>

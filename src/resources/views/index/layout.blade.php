...
<head>
  ...
  @if ($lang === 'zh')
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@100..900&family=Noto+Sans+SC:wght@100..900&display=swap"
          rel="stylesheet">
  @elseif ($lang === 'ar')
...
<body
  ...
  @php
        ...
        $floatingQuoteLabel = $localizedText('home_b2b.hero_cta_quote', [
            ...
            'es' => 'Obtener cotizacion gratis',
            'ru' => 'Получить расчет',
            'ar' => 'احصل على عرض سعر',
        ]);
        $floatingQuoteLabelCompact = $localizedText('layout.floating_quote_compact', [
            'en' => 'Free Quote',
            'zh' => '免费报价',
            'fr' => 'Devis',
            'es' => 'Cotizacion',
        ...
        $floatingWhatsappCtaLabel = $localizedText('layout.floating_whatsapp_cta', [
            'en' => 'WhatsApp Chat',
            'zh' => 'WhatsApp咨询',
            'fr' => 'Chat WhatsApp',
        ...
        $floatingWhatsappLabelCompact = $localizedText('layout.floating_whatsapp_compact', [
            'en' => 'WhatsApp',
            'zh' => 'WhatsApp咨询',
            'fr' => 'WhatsApp',
            'es' => 'WhatsApp',
  ...
  <header class="sticky bg-yideli-dark top-0 z-50 backdrop-blur-sm transition-all duration-300 shadow-md"
    ...
    <div
         class="max-w-[1200px] min-[1921px]:max-w-[1600px] min-[2561px]:max-w-[2400px] mx-auto flex h-16 items-center justify-between px-4 sm:h-20 sm:px-6 lg:px-12">
      <div class="me-4 flex-shrink-0 lg:me-8">
        <a class="block"
           href="{{ route('index', ['lang' => $lang]) }}">
          <img class="h-20 w-auto object-contain sm:h-24 lg:h-28"
               src="{{ asset('images/logo_big.jpg') }}"
               alt="Yideli Logo">
        </a>
      </div>

      <nav class="hidden text-sm font-medium text-[rgb(239,245,230)] lg:mt-8 lg:flex {{ in_array($lang, ['zh', 'ar']) ? 'gap-12 tracking-widest' : 'gap-6 tracking-wide' }}">
      ...
      <div class="flex items-center gap-3 text-[rgb(239,245,230)] sm:gap-4 lg:mt-8">
        <div class="relative"
             x-data="{
                 open: false,
    ...
    <div class="absolute start-0 top-full w-full max-h-[calc(100svh-4rem)] overflow-y-auto bg-yideli-base border-t border-yideli-line shadow-lg lg:hidden"
         x-show="mobileMenu"
         x-transition
         x-cloak>
  ...
  <div class="fixed bottom-3 end-3 z-40 flex flex-col items-end gap-2 sm:bottom-5 sm:end-5">
    <a class="inline-flex h-12 min-w-[132px] items-center justify-center gap-2.5 whitespace-nowrap rounded-full bg-[#25D366] px-4 text-xs font-semibold text-white shadow-[0_14px_40px_rgba(0,0,0,0.22)] transition hover:bg-[#1ebe5d] focus:outline-none focus:ring-2 focus:ring-[#25D366]/30 focus:ring-offset-2 sm:h-auto sm:w-[204px] sm:px-5 sm:py-3.5 sm:text-[13px]"
      ...
      <span class="sm:hidden">{{ $floatingQuoteLabelCompact }}</span>
      <span class="hidden sm:inline">{{ $floatingQuoteLabel }}</span>
    </a>

    <div class="group relative flex flex-col items-end">
      <div class="pointer-events-none absolute bottom-full end-0 mb-2 hidden translate-y-2 opacity-0 transition duration-200 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:translate-y-0 group-focus-within:opacity-100 sm:block">
        <div class="rounded-2xl bg-white p-2 shadow-[0_14px_40px_rgba(0,0,0,0.18)]">
          <img class="h-auto w-[210px] max-w-none"
               src="{{ asset('images/whatsapp-qrcode.png') }}"
      ...
      <a class="inline-flex h-12 min-w-[132px] items-center justify-center gap-2.5 whitespace-nowrap rounded-full bg-[#25D366] px-4 text-xs font-semibold text-white shadow-[0_14px_40px_rgba(0,0,0,0.22)] transition hover:bg-[#1ebe5d] focus:outline-none focus:ring-2 focus:ring-[#25D366]/30 focus:ring-offset-2 sm:h-auto sm:w-[204px] sm:px-5 sm:py-3.5 sm:text-[13px]"
        ...
        <span class="sm:hidden">{{ $floatingWhatsappLabelCompact }}</span>
        <span class="hidden sm:inline">{{ $floatingWhatsappCtaLabel }}</span>
      </a>
    </div>
  </div>

  <footer class="bg-yideli-dark pb-8 pt-14 text-white sm:pb-10 sm:pt-20">

    <div
         class="max-w-[1200px] min-[1921px]:max-w-[1600px] min-[2561px]:max-w-[2400px] mx-auto grid grid-cols-1 gap-10 border-b border-white/20 px-6 pb-12 md:grid-cols-2 lg:grid-cols-5 lg:px-12 lg:pb-16">
      ...
      <div class="lg:col-span-2">
        ...
        <p class="mb-6 text-sm leading-relaxed text-white/70 lg:pe-8">
          {!! nl2br($settings->company_name[$lang]) !!}
        </p>
        <div class="text-white/70 text-sm space-y-2">
          <p class="flex items-center gap-2">
            ...
            {!! nl2br($settings->contact_address[$lang]) !!}
          </p>
          <p class="flex items-center gap-2">
      ...
      <div>
        <h4 class="font-bold mb-6 text-sm uppercase tracking-widest">{!! nl2br(__('layout.footer_products')) !!}</h4>
        <ul class="space-y-4 text-sm text-white/70">
          @foreach ($nav_categories as $category)
            <li><a class="hover:text-white transition"
                 href="{{ route('product.index', ['lang' => $lang, 'slug' => $category->slug]) }}">{!! nl2br($category->name) !!}</a>
            </li>
          @endforeach
        </ul>
      </div>
      ...
      <div>
        <h4 class="font-bold mb-6 text-sm uppercase tracking-widest">{!! nl2br(__('layout.footer_follow_us')) !!}</h4>
        ...
        <div class="flex items-center gap-2 text-sm">
          ...
          @if ($whatsappHref)
            <a class="hover:text-white transition"
               href="{{ $whatsappHref }}"
               target="_blank"
               rel="noopener noreferrer">{{ $settings->contact_whatsapp }}</a>
          @else
    ...
    <div
         class="max-w-[1200px] min-[1921px]:max-w-[1600px] min-[2561px]:max-w-[2400px] mx-auto mt-8 flex flex-col items-center justify-between gap-4 px-6 text-center text-xs text-white/40 md:flex-row md:text-left lg:px-12">
      <p>&copy; 2025 Yideli Stationery. {!! nl2br(__('layout.footer_rights_reserved')) !!}</p>
      <div class="flex flex-wrap justify-center gap-4 md:mt-0 md:justify-end md:gap-6">
